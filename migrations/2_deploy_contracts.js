...
module.exports = function(deployer, network, accounts) {
  deployer.then(async () => {
    ...
    const medianizer = { address: '0xBC32048D9Be3A3c3b626c44D296CEfaaA400c797' } // MAINNET

    // const onDemandSpv = { address: '0x7c7ca88944a6cb439c0dc43c6406f00feb1e17b0' } // KOVAN

    // // Deploy Atomic Loan Contracts
    await deployer.deploy(Funds, usdc.address, '6');
    var funds = await Funds.deployed();
    await funds.setCompound(cusdc.address, comptroller.address);
    await deployer.deploy(Loans, funds.address, medianizer.address, usdc.address, '6');
    var loans = await Loans.deployed();
    await deployer.deploy(Sales, loans.address, funds.address, medianizer.address, usdc.address);
    ...
    console.info(`USDC_LOAN_FUNDS_ADDRESS=${funds.address}`)
    console.info(`USDC_LOAN_LOANS_ADDRESS=${loans.address}`)
    console.info(`USDC_LOAN_SALES_ADDRESS=${sales.address}`)
    ...
    console.info(`  "USDC_FUNDS": "${funds.address}",`)
    console.info(`  "USDC_LOANS": "${loans.address}",`)
    console.info(`  "USDC_SALES": "${sales.address}",`)
    ...
    const contractAddresses = {
        ...
        // "CDAI": cdai.address,
        "CUSDC": cusdc.address,
        "USDC_FUNDS": funds.address,
        "USDC_LOANS": loans.address,
        "USDC_SALES": sales.address,
