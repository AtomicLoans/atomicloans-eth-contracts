...
function getCollateralSatAmounts(collateralValue, owedToLender, btcPrice) {
  const seizableValue = Math.ceil(BigNumber(fromWei(owedToLender.toString())).dividedBy(btcPrice).times(BTC_TO_SAT).toString())
...
contract("E2E", accounts => {
  ...
  describe('Regular loan flow with repayment before loanExpiration', function() {
    it('should request, lock, approve, withdraw, repay, accept, unlock', async function() {
      ...
      const collateralSatValues = [collateral, owedToLender, btcPrice]

      const values = getCollateralSatAmounts(collateral, owedToLender, btcPrice);
